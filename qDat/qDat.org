#+begin_export latex
\section{Real Data}
Next we do real genomics by detecting unique regions in real data. Our
example data set is the smallest human chromosome, chromosome 21. We
download the reference assembly of the human genome and its
annotations in ``dehydrated'' form.
#+end_export
...
#+end_src
#+begin_export latex
We unzip the data set and rehydrate it. Since we are downloading 4.6
GB of data, this can take a while, a bit over two minutes in our case.
#+end_export
#+begin_src sh <<qDat.sh>>=
...
t       4866512 0.104
\end{verbatim}
The lower case residue designations indicate repeats, while \ty{N}
indicates an unknown nucleotide. 14.2\% of chromosome 21 are still
unknown.
#+end_export
#+begin_export latex
...
we remove and then re-wrap the sequence before we store it as
\ty{chr21r.fna}.
#+end_export
#+begin_src sh <<qDat.sh>>=
  tr -d N < chr21.fna |
...
#+end_src
#+begin_export latex
We carry out the sliding window analysis and pick the 5\% quantile of
mappability values. Given that there are $46.7\times 10^6$ mappability
values, the 5\% quantile is entry number
\[
46.7\times 10^6/100*5\approx 2,335,000.
\]
This turns out to be 0.9273.
#+end_export
#+begin_src sh <<qDat.sh>>=
...
the gene encoding the transcription factor SIM2 located at
36,699,115--36,749,917. ``SIM2'' stands for ``single-minded 2'' and
was originally named for the morphological effect of its knockout on
the midline neuroepithelium of fruit flies, where two strands of
neurological tissue merge into one~\cite{tho88:mol}. SIM2 is a
transcription factor based on the helix-loop-helix motif and mouse
knock-outs are associated with severe morphological and neurological
abnormalities~\cite{but22:cha}. In humans, the gene is located in the
Down chromosomal region on chromosome 21~\cite{dah95:dow}, which is
critical for the development of Down syndrome. And while the exact
molecular details of SIM2 action during development and beyond remain
unclear, it is known that patients with intellectual disabilities have
numerous amino acid changes in SIM2 that \emph{in vitro} impair its
function as a transcription factor~\cite{but22:cha}.
...
#+end_src
#+begin_export latex
Figure~\ref{fig:chr21} shows the plot of our region. The mappability
...
developed. If we lowered the mappability threshold from 0.9273 to
...
\begin{figure}
  \begin{center}
    \includegraphics[width=0.8\textwidth]{../qDat/chr21_clean}
  \end{center}
  \caption{Two measures of uniqueness along a portion of human
    chromosome 21, $\cm$ (\emph{c}) and the mappbaility (\emph{m});
    shown are also the $\cm$ threshold (\emph{tc}), the mappability
    threshold (\emph{tm}), the unique region picked based on this threshold
    (\emph{u}), and the SIM2 gene that intersects this region (\emph{s}).}\label{fig:chr21}
\end{figure}
#+end_export
#+begin_src sh <<qDat.sh>>=
