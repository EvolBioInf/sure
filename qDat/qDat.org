...
our 31 focal complexity regions have a 99\,\% overlap with the
mappability regions. In other words, the 31 complexity regions that do
overlap, overlap completely.
#+end_export
#+begin_src sh <<qDat.sh>>=
  bedtools intersect -wo -a com.txt -b map.txt |
      awk '{l=$3-$2+1;t+=l;s+=$NF}END{print s/t*100}'
#+end_src
#+begin_export latex
\begin{verbatim}
99.28
