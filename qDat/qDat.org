...
\begin{figure}
  ...
  \caption{Two measures of uniqueness along a portion of human
    ...
    shown are also the $\cm$ threshold (\emph{t}), the unique region
    picked based on this threshold (\emph{u}), and the tree genes that
    intersect this region, LOC105369308 (\emph{g1}), SIM2 (\emph{g2}),
    and LOC107985492 (\emph{g3}).}\label{fig:chr21}
\end{figure}
#+end_export
#+begin_src sh <<qDat.sh>>=
