...
regions. To investigate what happens with $e=1$, we rerun the mapping
step, which takes 149\,s and 270\,MB of RAM, approximately seven times
longer than with zero mismatches, but with the same memory
requirement.
#+end_export
#+begin_src sh <<qDat.sh>>=
...
threshold for the $(15,1)$ data. For this purpose, we rerun the
...
2.0\,Mb with $(15,1)$ mappability, we should check the true uniqueness
of the regions we just picked.

\subsection*{Assessing Uniqueness}
To assess the uniqueness of the 2.0\,Mb picked with $(15, 1)$
mappbability, we cut them from chromosome 21, blast them against
...
the 107 regions we just picked by running the program \ty{cutSeq} on
every region.
#+end_export
#+begin_src sh <<qDat.sh>>=
...
\]
where the $\mbox{false prediction}$ is the number of bases outside the
original query, and the $\mbox{total prediction}$ is the length of the
query in base pairs. This calculation is implemented in the program
\ty{pf.awk}, which we still need to write. It says $f_{\rm p}$ is
10.94\,\% for $(15,1)$ mappability.
#+end_export
#+begin_src sh <<qDat.sh>>=
...
the 2.0\,Mb found with \ty{genmap}? To find out, we use \ty{bedtools}
...
#+end_export
#+begin_export latex
Of the 33 regions in \ty{com.txt}, 31 overlapped with a region in
\ty{map.txt}.
#+end_export
#+begin_src sh <<qDat.sh>>=
  wc -l inter.txt
#+end_src
#+begin_export latex
To find out the extent to which these 31 regions overlap, we note that
...
our 31 focal complexity regions sum to 419\,kb that have a 99\,\%
overlap with the mappability regions. In other words, the 31
complexity regions that do overlap, overlap completely.
#+end_export
#+begin_src sh <<qDat.sh>>=
...
#+end_src
#+begin_export latex
We make sure \ty{lone.txt} contains two regions.
#+end_export
#+begin_src sh <<qDat.sh>>=
  cat lone.txt
#+end_src
...
#+end_export
#+begin_export latex
The lone regions amount to 21\,kb.
#+end_export
#+begin_src sh <<qDat.sh>>=
  awk '{l=$3-$2+1;s+=l}END{print s}' lone.txt
#+end_src
#+begin_export latex
So it seems as if \ty{macle} has uncovered 21 unique kb in chromosome
21 not contained in the mappability output. Given that the other 31
complexity regions are a virtual subset of the mappability result, we
should check the uniqueness of the two lone regions. So we cut them
...
#+end_src
#+begin_export latex
The false prediction rate for the lone regions is 0.6\,\%, which is
gratifyingly low. So these are genuine unique regions not contained in
the mappability output.
#+end_export
#+begin_src sh <<qDat.sh>>=
...
Total:                 21000  
False:                   122  
False_prediction_rate:  0.58 %
\end{verbatim}
From the putative unique regions we pick the highest scoring one,
which consists of the 26\,kb between 36,688,001 and 36,714,000. Not
surprisingly given our overlap analysis, this highest scoring
complexity region is fully contained in the 31\,kb of the highest
scoring mappability region.
#+end_export
#+begin_src sh <<qDat.sh>>=
  sort -n -k 4 com.txt |
      tail -n 1
...
generated. Figure~\ref{fig:chr21} shows that the $(15,1)$ mappability
and complexity are closely correlated. We also see that based on
$(15,1)$ mappability we predict more unique regions than based on
complexity.
\begin{figure}
  \begin{center}
